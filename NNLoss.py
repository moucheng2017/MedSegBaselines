...
def f_beta_loss(input, target, beta=1.0):
    smooth = 0.01
    iflat = input.view(-1)
    tflat = target.view(-1)
    numerator = (1.0 + beta**2)*(iflat*tflat)
    denominator = (1.0 + beta**2)*(iflat*tflat) + beta**2*(iflat*(1.0 - tflat)) + (1.0 - iflat)*tflat
    fbeta = (numerator.sum() + smooth) / (denominator.sum() + smooth)
